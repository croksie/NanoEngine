...
VulkanShader::VulkanShader(VkDevice device, ShaderType type, const std::vector<uint32_t>& spirvCode, const std::string &entryPoint)
    : m_device(device), m_entryPoint(entryPoint), m_type(type)
{
    ENGINE_LOG_TRACE("VulkanShader::Creating shader...");
    ...
    : m_device(device), m_entryPoint(entryPoint), m_type(type)
{
    ...
    if (result != VK_SUCCESS){
        ENGINE_LOG_CRITICAL("VulkanShader::Failed to create shader module. Vulkan error Code : {}", static_cast<int>(result));
    ...
    const std::string& entryPoint)
{
    ...
    {
        ENGINE_LOG_ERROR("VulkanShader::Failed to compile shader : {}", result.GetErrorMessage());
        return {};
    }
