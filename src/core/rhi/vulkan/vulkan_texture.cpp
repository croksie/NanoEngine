...
VulkanTexture::VulkanTexture(const TextureDesc& desc, vulkan::VulkanContext& ctx) : m_desc(desc), m_ctx(&ctx) {
    ENGINE_LOG_INFO("VulkanTexture::Creating texture...");
    ...
    imageInfo.format = bufferTypeToVulkanType(m_desc.format);
    ...
    if (vkCreateImage(m_ctx->device, &imageInfo, nullptr, &m_image) != VK_SUCCESS) {
        ENGINE_LOG_CRITICAL("VulkanTexture::Failed to create image");
    ...
    allocInfo.memoryTypeIndex = vulkan::findMemoryType(*m_ctx, memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    if (vkAllocateMemory(m_ctx->device, &allocInfo, nullptr, &m_imageMemory) != VK_SUCCESS) {
        ENGINE_LOG_CRITICAL("VulkanTexture::Failed to allocate image memory");
        throw std::runtime_error("Failed to allocate image memory");
    }
    vkBindImageMemory(m_ctx->device, m_image, m_imageMemory, 0);

    // 3. Staging buffer and copy
    if (desc.initialData != nullptr) {
        ...
        VkCommandBuffer cmd = vulkan::beginSingleTimeCommands(*m_ctx);
        ...
        vulkan::endSingleTimeCommands(*m_ctx, cmd);
    }
    ...
    viewInfo.format = bufferTypeToVulkanType(m_desc.format);
    ...
    if (vkCreateImageView(m_ctx->device, &viewInfo, nullptr, &m_imageView) != VK_SUCCESS) {
        ENGINE_LOG_CRITICAL("VulkanTexture::Failed to create image view");
    ...
    if (vkCreateSampler(m_ctx->device, &samplerInfo, nullptr, &m_textureSampler) != VK_SUCCESS) {
        ENGINE_LOG_CRITICAL("VulkanTexture::Failed to create sampler");
        throw std::runtime_error("Failed to create sampler");
    }

    ENGINE_LOG_INFO("VulkanTexture::Texture created successfully");
}
